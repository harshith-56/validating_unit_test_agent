...
describe('fetchWithRetry', () => {
  ...
  it('throws error after retries when client.get always rejects', async () => {
    ...
    await expect(fetchWithRetry(client, 'http://test.url', 1)).rejects.toThrow(`Failed after retries: ${error}`)
  ...
  it('throws error after retries when client.get returns non-200 status', async () => {
    ...
    await expect(fetchWithRetry(client, 'http://test.url', 2)).rejects.toThrow('Failed after retries: Error: Bad response')
