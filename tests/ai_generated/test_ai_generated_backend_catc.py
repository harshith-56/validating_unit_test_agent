...
def test_is_rate_limited_below_limit():
    with patch('backend.catc._rate_limit', {'user1': 2}) as mock_rate_limit:
        result = is_rate_limited('user1', 5)
        assert result is False
        assert mock_rate_limit['user1'] == 3

def test_is_rate_limited_at_limit():
    with patch('backend.catc._rate_limit', {'user2': 3}) as mock_rate_limit:
        result = is_rate_limited('user2', 3)
        assert result is True
        assert mock_rate_limit['user2'] == 3
