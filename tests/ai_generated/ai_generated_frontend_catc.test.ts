...
describe('createSession', () => {
  ...
  test('handles token as string "null"', () => {
    const token = 'null'
    const userId = 3
    const result = createSession(token, userId)
    expect(result).toBe(true)
  ...
  test('handles token as string "undefined"', () => {
  ...
  test('throws when token is null (invalid input)', () => {
...
describe('isRateLimited', () => {
  ...
  test('returns false and increments count when user is below limit', () => {
    catcModule.rateLimit['user1'] = 2
    const result = isRateLimited('user1', 5)
    expect(result).toBe(false)
    expect(catcModule.rateLimit['user1']).toBe(3)
  })

  test('returns true when user count equals limit', () => {
    catcModule.rateLimit['user2'] = 3
    const result = isRateLimited('user2', 3)
    expect(result).toBe(true)
    expect(catcModule.rateLimit['user2']).toBe(3)
  })
  ...
  test('handles empty string userId as valid key', () => {
  ...
  test('handles non-string userId by coercing to string key', () => {
    ...
    const result = isRateLimited(123, 1)
    expect(result).toBe(false)
    // @ts-expect-error accessing with number key
    expect(catcModule.rateLimit[123]).toBe(1)
  })

  test('handles null userId by coercing to string "null"', () => {
    // @ts-expect-error testing invalid input type
    const result = isRateLimited(null, 1)
    expect(result).toBe(false)
    // @ts-expect-error accessing with null key
    expect(catcModule.rateLimit['null']).toBe(1)
  })
})
